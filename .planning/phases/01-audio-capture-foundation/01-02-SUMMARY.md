...
plan: 02
subsystem: audio
tags: [sounddevice, numpy, soxr, wasapi, portaudio, audio-capture]

# Dependency graph
requires: []
provides:
  - Device enumeration with WASAPI detection
  - Loopback capability probing for output devices
  - Microphone capture source using WASAPI
  - System audio capture source interface
  - FakeAudioModule for deterministic testing
affects:
  - 01-03-session-wiring
  - 02-transcription-engine

# Tech tracking
tech-stack:
  added:
    - sounddevice>=0.4.6 (WASAPI via PortAudio)
    - numpy>=1.24.0 (frame conversion)
    - soxr>=0.3.0 (resampling)
    - comtypes>=1.2.0 (Windows COM for Core Audio)
  patterns:
    - Queue-based audio frame delivery (producer-consumer)
    - WASAPI-first device selection on Windows
    - Fake/Stub pattern for testing without hardware
...
key-decisions:
  - "sounddevice.WasapiSettings doesn't expose loopback parameter - documented limitation"
  - "Loopback probing detects WASAPI output devices as potentially loopback-capable"
  - "MicSource validates WASAPI requirement on Windows for AUD-06 compliance"
  - "FakeAudioModule provides WAV-file-driven deterministic testing"

patterns-established:
  - "Audio sources use queue.Queue for thread-safe frame delivery"
  - "Sources expose consistent API: start(), stop(), read_frames(), get_metadata()"
  - "Non-blocking callback with frame dropping on queue full"
  - "Fail-fast validation for non-WASAPI devices on Windows"
