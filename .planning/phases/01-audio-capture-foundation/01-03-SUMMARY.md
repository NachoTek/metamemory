...
plan: 03
subsystem: audio
tags: [audio, session, recording, mixing, resampling, cli]

# Dependency graph
requires:
  - phase: 01-01
    provides: "PcmPartWriter for streaming audio to disk"
  - phase: 01-02
    provides: "MicSource, SystemSource, FakeAudioModule for audio capture"
provides:
  - AudioSession for managing recording lifecycle
  - Multi-source mixing (mic + system audio)
  - Automatic resampling to 16kHz via soxr
  - Stereo to mono downmixing
  - CLI smoke harness for recording without UI
  - Automated end-to-end tests via FakeAudioModule
affects:
  - 01-04-file-streaming
...
tech-stack:
  added: []
  patterns:
    - "Producer-consumer pattern with consumer thread"
    - "AudioSourceWrapper for per-source resampling"
    - "Session state machine (IDLE -> RECORDING -> FINALIZED)"
    - "Queue-based bounded memory with frame dropping"

key-files:
  created:
    - src/metamemory/audio/session.py - Session orchestration
    - src/metamemory/audio/cli.py - CLI smoke harness
    - tests/test_audio_session.py - End-to-end tests
  modified:
    - src/metamemory/audio/__init__.py - Export session API

key-decisions:
  - "Use soxr.ResampleStream for streaming resampling (not one-shot)"
  - "Mix sources by summing float32 arrays, clip to [-1, 1]"
  - "Downmix stereo to mono by averaging channels"
  - "Consumer thread pattern for non-blocking frame processing"
  - "FakeAudioModule with loop=True for deterministic testing"

patterns-established:
  - "SessionConfig + SourceConfig dataclasses for type-safe configuration"
  - "AudioSourceWrapper abstracts resampling/mixing per source"
  - "SessionStats for tracking frames recorded/dropped"
  - "CLI uses PYTHONPATH manipulation for imports without package install"
