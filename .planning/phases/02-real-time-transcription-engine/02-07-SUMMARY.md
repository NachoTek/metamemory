...
subsystem: transcription
 tags: [whisper, deduplication, segment-tracking, real-time]

# Dependency graph
requires:
  - phase: 02-real-time-transcription-engine
    provides: AccumulatingTranscriptionProcessor with buffer management
provides:
  - Segment-level deduplication to prevent transcript repetition
  - _last_emitted_segment_index tracking in processor
  - Clean separation: processor handles dedup, controller appends
affects:
  - Phase 2 completion
  - UAT Test 9 (transcript accuracy)

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "Segment index tracking for incremental emission"
    - "Processor-level deduplication with controller-level appending"
...
key-decisions:
  - "Track last emitted segment index instead of text-based deduplication"
  - "Reset segment index on silence detection (phrase boundaries)"
  - "Keep deduplication logic in processor, controller remains simple"

patterns-established:
  - "Segment filtering: only emit segments with index > last_emitted"
  - "State reset on phrase boundaries for clean transcript lines"
