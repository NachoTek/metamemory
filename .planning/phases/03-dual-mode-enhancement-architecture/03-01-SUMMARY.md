...
tags: enhancement, queue, worker-pool, asyncio, ThreadPoolExecutor, bounded-queue

# Dependency graph
requires:
  - phase: 02-real-time-transcription-engine
    provides: confidence scoring, streaming pipeline, real-time transcription
provides:
  - Bounded enhancement queue for low-confidence segments
  - Async worker pool for background enhancement processing
  - Large model inference stub (EnhancementProcessor)
  - Enhancement configuration model (EnhancementSettings)
  - Enhancement status display in UI
  - Enhancement configuration UI (threshold, workers, model selection)
affects:
  - 03-02-large-model-integration (uses enhancement queue and worker pool)
  - 03-03-async-worker-pool (extends worker pool functionality)
  - 03-04-ui-enhancements (uses bold formatting for enhanced segments)

# Tech tracking
tech-stack:
  added:
    - asyncio (async/await for concurrent processing)
    - concurrent.futures.ThreadPoolExecutor (parallel worker execution)
    - queue.Queue (thread-safe bounded queue)
    - dataclasses (type-safe configuration models)
  patterns:
    - Bounded queue pattern with graceful degradation
    - Async worker pool with thread executor
    - Confidence-based selective enhancement
    - Real-time transcript update mechanism

key-files:
  created:
    - src/metamemory/transcription/enhancement.py (EnhancementQueue, EnhancementWorkerPool, EnhancementProcessor, TranscriptUpdater)
  modified:
    - src/metamemory/config/models.py (EnhancementSettings dataclass)
    - src/metamemory/transcription/streaming_pipeline.py (enhancement integration)
    - src/metamemory/widgets/floating_panels.py (enhancement status and configuration UI)
    - src/metamemory/widgets/main_widget.py (enhancement settings signal handling)

key-decisions:
  - "Bounded queue (100 segments max) prevents memory exhaustion during long recordings"
  - "Asyncio + ThreadPoolExecutor for parallel processing without blocking main thread"
  - "Confidence threshold 70% default balances coverage and resource usage"
  - "4 workers default optimal for consumer-grade hardware"
  - "Bold formatting for enhanced segments provides visual distinction"
